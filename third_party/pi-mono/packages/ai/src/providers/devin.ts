...
import { isDemotedThinking } from "../utils/block-symbols.ts";
import { deterministicUuid } from "../utils/deterministic-id.ts";
...
import { normalizeSchemaForGoogle, toolWireSchema } from "../utils/schema-normalize.ts";
...
import {
	...
	StopReason,
} from "./devin/devin-proto.ts";
import { create, fromBinary, toBinary } from "./devin/protobuf.ts";
import { DEVIN_DEFAULT_BASE_URL, decodeDevinUnaryMessage, devinCliMetadata } from "./devin/wire.ts";
...
export interface DevinOptions extends StreamOptions {
	...
	chatModelUid?: string;
	/** Extra stop patterns appended to the Cascade defaults. */
...
/**
 * `developer`-role messages are not part of the vendored `Message` union, but
 * hosts may still thread them through `Context.messages`; the Devin transport
 * flattens them into USER prompts like upstream does.
 */
...
type DevinMessage = Message | DeveloperMessage;

/** Compat flags the Devin transport reads; vendored `Model.compat` is `never` for this api. */
interface DevinModelCompat {
	/** Model is a server-side router (`adaptive`) resolved through AssignModel. */
	modelRouter?: boolean;
	supportsParallelToolCalls?: boolean;
}

interface DevinModelExtras {
	compat?: DevinModelCompat;
	requestModelId?: string;
}

function devinCompat(model: Model<"devin-agent">): DevinModelCompat {
	return (model as Model<"devin-agent"> & DevinModelExtras).compat ?? {};
}

function devinRequestModelId(model: Model<"devin-agent">): string | undefined {
	return (model as Model<"devin-agent"> & DevinModelExtras).requestModelId;
}

/** Heuristic stand-in for catalog `classifyModel`: Devin's Gemini-backed uids all carry "gemini". */
function isGeminiModelId(modelId: string): boolean {
	return /gemini/i.test(modelId);
}

interface DevinUsage extends Usage {
	credits?: {
		cost?: number;
...
interface DevinAssistantMessage extends AssistantMessage {
	usage: DevinUsage;
	/** Concrete upstream model uid when the router landed on a different model. */
...
// ANSI escape sequences (CSI/OSC) — portable stand-in for Bun.stripANSI.
...
function toWellFormed(text: string): string {
	return text.replace(/[\uD800-\uDBFF](?![\uDC00-\uDFFF])|(^|[^\uD800-\uDBFF])[\uDC00-\uDFFF]/g, `$1${REPLACEMENT_CHAR}`);
}
...
function sanitizeText(text: string): string {
	...
	const input = wellFormed !== text ? wellFormed.replaceAll(REPLACEMENT_CHAR, "") : text;
	CONTROL_RE.lastIndex = 0;
	if (CONTROL_RE.exec(input) === null) return input;
	const stripped = input.indexOf("\x1b") === -1 ? input : input.replace(ANSI_PATTERN, "");
	CONTROL_RE.lastIndex = 0;
...
function createDevinHttpError(operation: string, response: Response, payload: Uint8Array): ApiHttpError {
	const status = `${response.status}${response.statusText ? ` ${response.statusText}` : ""}`;
	const detail = devinErrorDetail(response, payload);
	return new ApiHttpError(`Devin ${operation} error ${status}${detail ? `: ${detail}` : ""}`, response.status, {
	...
	(async () => {
		...
		try {
			...
			const turn: DevinTurn = {
				...
				messages: transformMessages(context.messages, model) as DevinMessage[],
			};
			...
			if (devinCompat(model).modelRouter) {
			...
			if (!response.body) {
				throw new ProviderResponseError("Devin API error: response body is empty", {
					provider: model.provider,
					kind: "empty-body",
				});
			}
			...
			for (;;) {
				...
				while (pending.length >= 5) {
					...
					if (flag & CONNECT_END_STREAM_FLAG) {
						...
						if (trailerError) {
							// These rejections carry no HTTP error body, so the raw
							...
							const error = new ValidationError(trailerError.formatted);
							if (
								...
								const lastRole = (context.messages as DevinMessage[]).at(-1)?.role;
								if (lastRole === "user" || lastRole === "developer") {
									...
									if (lastRole === "developer") {
										const messages = context.messages as DevinMessage[];
...
async function fetchDevinAuthMetadata(
	...
	if (!decoded?.userJwt) {
		throw new ProviderResponseError("Devin auth error: GetUserJwt returned an empty user JWT", {
			provider: "devin",
			kind: "runtime",
		});
	}
...
async function assignDevinModel(
	...
	const request = create(AssignModelRequestSchema, {
		...
		modelRouterUid: devinRequestModelId(model) ?? model.id,
	...
	logger.debug("devin: router assigned a model", {
		router: devinRequestModelId(model) ?? model.id,
 ...
 */
function buildRouterPrompt(messages: DevinMessage[]): ChatMessagePrompt | undefined {
...
/**
 * Build a `GetChatMessageRequest` for one Cascade turn. Auth rides inside
...
function buildDevinChatRequest(
	...
	const chatModelUid = assignment?.modelUid ?? options?.chatModelUid ?? devinRequestModelId(model) ?? model.id;
	...
	// prefix is the server's own enum namespace, which the id heuristic cannot parse.
	...
	const tools = (context.tools ?? []).map((tool: Tool) => {
		...
		return create(ChatToolDefinitionSchema, {
			...
			strict: (tool as { strict?: boolean }).strict ?? false,
		});
	});
	return create(GetChatMessageRequestSchema, {
		...
		disableParallelToolCalls: !devinCompat(model).supportsParallelToolCalls,
		cascadeId: turn.cascadeId,
...
/** Map `Message` history onto Cascade `ChatMessagePrompt`s (USER / SYSTEM / TOOL channels). */
 ...
 * records). Intermittent `invalid_argument` rejections arrive as end-of-stream
 * trailers with no HTTP error body, so any detail payload here is the only
...
/**
 * `SimpleStreamOptions` entry point for registry registration. Devin has no
 * thinking-level knob; reasoning effort is a server-side concern, so this just
 * forwards the shared base options.
 */
export const streamSimpleDevin: StreamFunction<"devin-agent", SimpleStreamOptions> = (
	...
	return streamDevin(model, context, buildBaseOptions(model, options, options?.apiKey));
};
