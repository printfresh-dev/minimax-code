import { AbortError, StreamTimeoutError } from "./provider-errors.ts";
...
export function getStreamFirstEventTimeoutMs(
	...
	return normalizeIdleTimeoutMs(envValue("PI_STREAM_FIRST_EVENT_TIMEOUT_MS"), fallback);
}
 ...
 * pre-response request. This arms a `clearTimeout`-able timer instead;
...
export interface IdleTimeoutIteratorOptions {
	 ...
	 * transport is waiting on a server-requested local tool bridge before
	 * anything can flow upstream again. While it returns true, an expired idle /
	 * first-item deadline slides forward instead of aborting — the silence is
	 * ours, not a provider stall. The watchdog re-arms with a full budget once
	 * the local work completes, so a provider that stalls afterwards is still
	 * caught.
	 */
	hasPendingLocalWork?: () => boolean;
	/**
	 * Cancel iteration as soon as this signal aborts. Required for caller-driven
	 * cancellation when the underlying transport does not surface signal aborts
	 * to the iterator (HTTP/2 proxies, native sockets, mocked fetch). Without
	 * this, the consumer sleeps on iterator.next() until the idle/first-event
	 * watchdog fires.
	 */
...
export async function* iterateWithIdleTimeout<T>(
	...
	if (abortSignal) {
		const { promise, resolve } = promiseWithResolvers<{ kind: "abort" }>();
	...
	const armTimer = (deadlineMs: number): void => {
		if (timeoutPromise === undefined || timeoutFired) {
			...
			const { promise, resolve } = promiseWithResolvers<{ kind: "timeout" }>();
			...
			if (awaitingFirstItem) {
				if (firstItemDeadlineMs !== undefined) {
					...
					if (activeTimeoutMs <= 0) {
						if (!hasPendingLocalWork()) {
							...
							closeIterator();
							throw new StreamTimeoutError(options.firstItemErrorMessage ?? options.errorMessage);
						}
				...
				if (activeTimeoutMs <= 0) {
					if (!hasPendingLocalWork()) {
						options.onIdle?.();
						closeIterator();
						throw new StreamTimeoutError(options.errorMessage);
					}
					extendDeadlineForLocalWork();
